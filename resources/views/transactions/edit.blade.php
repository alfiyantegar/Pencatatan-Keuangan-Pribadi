...
@section('content')
    ...
    <form action="{{ route('transactions.update', $transaction) }}" method="POST" class="bg-white p-6 rounded shadow">
        ...
        <div class="mb-4">
            ...
            <select name="category_id" id="category_id" class="w-full border p-2 rounded" required>
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}" {{ $category->id == $transaction->category_id ? 'selected' : '' }} data-photo="{{ $category->photo ? asset('storage/' . $category->photo) : '' }}">
        ...
        <div class="mb-4">
            ...
            <img id="category_photo_preview" src="{{ $transaction->category->photo ? asset('storage/' . $transaction->category->photo) : '' }}" alt="Category Photo" class="w-24 h-24 object-cover rounded {{ $transaction->category->photo ? '' : 'hidden' }}">
        </div>
        <div class="mb-4">
